...
export default function Page() {
  ...
  async function claimFaucet() {
    writeContract({
      address: "0x4126fc52ce3a6443420f5dbe787d3eefb4ef3ca8",
      abi: ABI_FAUCET,
          ...
          {gameState === "playing" &&
            ...
            shapes.map((shape, index) => (
              <div
                key={index}
                className={`${
                  SHAPE_STYLES[shape.type]
                } absolute cursor-pointer transform hover:scale-110 active:scale-95 transition-all duration-200`}
                style={{
          ...
          {gameState === "success" && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-900/90 backdrop-blur-sm">
              <div className="flex flex-col items-center space-y-4 text-center">
                <h2 className="text-xl md:text-3xl font-bold text-green-500 animate-bounce">
                  Verification Complete!
                </h2>
                <button
                  disabled={isPending}
                  onClick={() => claimFaucet()}
                  className="px-6 py-3 text-white bg-green-600 hover:bg-green-700 rounded-lg shadow-lg font-semibold transition duration-300"
                >
                  {isPending ? "Claiming..." : "Claim"}
                  {isConfirming && <div>Waiting for confirmation...</div>}
                  {isConfirmed && <div>Successfully Claimed</div>}
                  {error && (
                    <div className="text-red-500 font-semibold bg-red-100 p-4 rounded-lg shadow-md">
